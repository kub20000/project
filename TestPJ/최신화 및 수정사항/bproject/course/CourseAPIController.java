...
public class CourseAPIController {
    ...
    public ResponseEntity<CourseDetailDto> getCourseDetails(@PathVariable int courseId,
        ...
        if (loginUser == null) {
            // 로그인되어 있지 않다면 401 Unauthorized 응답
        ...
        // 2. 유저 ID 및 시청 시간 조회 (로그인이 이미 검증됨)
        int userId = loginUser.getId();

        //  CourseService를 통해 유저가 마지막으로 시청한 시간(초)을 조회
        int userDurationSec = courseService.getUserDurationSecForClient(userId, courseId);
