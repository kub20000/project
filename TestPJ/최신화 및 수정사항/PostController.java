...
public class PostController {
    ...
    public String addPost(@ModelAttribute Post post, HttpSession session) { // HttpSession 추가
        ...
        String userRole = String.valueOf(loginUser.getRole());

        if (userRole != null && "ADMIN".equalsIgnoreCase(userRole.trim())) {
            // 관리자(ADMIN)일 경우, '/manager/postM' 페이지로 리다이렉트
            return "redirect:/manager/postM";
        } else {
            // 일반 사용자일 경우, 기존 '/post/list' 페이지로 리다이렉트
    ...
    public String postDetailForm(@PathVariable int id, Model model, HttpSession session){
    ...
    public String postEditForm(@PathVariable int id, Model model, HttpSession session){
        Post post = (Post) postService.findById(id).orElseThrow(
